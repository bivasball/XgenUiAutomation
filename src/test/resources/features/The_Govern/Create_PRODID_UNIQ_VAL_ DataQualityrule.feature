...
Feature: The Govern page and its functionality
  ...
  @TC_6001
  Scenario Outline: To Create PRODID_UNIQ_VAL Data Quality rule
    ...
    And Enter the input to Name "<NAME>", Description "<DESCRIPTION>" and Select Major Button
    ...
    Examples: 
      | NAME            | DESCRIPTION                  |
      | PRODID_UNIQ_VAL | Creating a Data quality Rule |

  @TC_6002
  Scenario Outline: To delete PRODID_UNIQ_VAL Data Quality rule
    When the user navigate to "http://xgendemo.uxli.com" url
    Then verify the user should be able to enter username and password
    When the user click on Login button
    Then verify the login should be successful and Select Subscription dropdown should be available
    ...
    And Navigate to Govern Page
    And Click on Models under Data Assets and Select XIL_DIM_PRODUCTS
    And Click on Data Quality tab and Verify that "<NAME>" is present or not
    Then Verify that "<NAME>" is deleted successfully
  ...
  @TC_6003
  Scenario Outline: To recreate PRODID_UNIQ_VAL Data Quality rule, if exists delete then recreate
    When the user navigate to "http://xgendemo.uxli.com" url
    Then verify the user should be able to enter username and password
    When the user click on Login button
    Then verify the login should be successful and Select Subscription dropdown should be available
